...
import {
  CreateDonationSectionDto,
  UpdateDonationSectionDto,
} from './dto/donation-section.dto';
...
export class DonationSectionService {
  ...
  async getOrCreateDefault(): Promise<DonationSection> {
    ...
    if (!section) {
      ...
      section = this.donationSectionRepository.create({
        ...
        description:
          '100% of your donation goes directly to communities in need.',
        ...
        impactItems: [
          {
            id: '1',
            amount: '$10',
            label: 'Provides school supplies for 1 child',
            icon: 'Star',
          },
          {
            id: '2',
            amount: '$25',
            label: 'Feeds a family for one week',
            icon: 'Heart',
          },
          {
            id: '3',
            amount: '$50',
            label: 'Clean water access for a month',
            icon: 'Target',
          },
          {
            id: '4',
            amount: '$100',
            label: 'Medical supplies for a clinic',
            icon: 'Users',
          },
        ],
        amountPresets: [
        ...
        thankYouMessage:
          'Thank you for your generosity! Your donation will help us make a real difference.',
        receiptEnabled: true,
      });
  ...
  async update(
    id: number,
    dto: UpdateDonationSectionDto,
  ): Promise<DonationSection> {
    const section = await this.donationSectionRepository.findOne({
      where: { id },
    });
    if (!section) {
