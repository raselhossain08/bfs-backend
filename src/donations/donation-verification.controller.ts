...
export class DonationVerificationController {
    ...
    async getDonationStats() {
        try {
            ...
            const withoutDonorId = allDonations.filter(d => d.donorId === null).length;
    ...
    async verifyUserDonations(@Request() req: any, @Query('userId') userId: string) {
        ...
        const targetUserId = isAdmin ? (userId ? parseInt(userId, 10) : requestingUserId) : requestingUserId;
