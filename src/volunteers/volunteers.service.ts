...
export class VolunteersService {
    ...
    if (existing) {
      ...
      if (rejectedDate) {
        ...
        if (daysSinceRejection < 30) {
          throw new BadRequestException(
            `Your previous application was rejected. Please wait ${30 - daysSinceRejection} days before reapplying.`,
          );
        }
      }
    }
  ...
  async getApplications(query: ApplicationQueryDto) {
    ...
    if (experience) {
      queryBuilder.andWhere('application.experience = :experience', {
        experience,
      });
    }

    if (startDate) {
      queryBuilder.andWhere('application.submittedAt >= :startDate', {
        startDate: new Date(startDate),
      });
    }

    if (endDate) {
      queryBuilder.andWhere('application.submittedAt <= :endDate', {
  ...
  async updateApplicationStatus(
    ...
    if (
      ...
      dto.status !== ApplicationStatus.PENDING
    ) {
      throw new BadRequestException(
  ...
  async bulkUpdateStatus(
    ...
    this.logger.log(
      `Bulk ${dto.status} for ${applications.length} applications by user ${reviewerId}`,
    );
  ...
  private async createVolunteerFromApplication(
  ...
  async getVolunteers(query: VolunteerQueryDto) {
    ...
    if (skill) {
      queryBuilder.andWhere('volunteer.skills ILIKE :skill', {
        skill: `%${skill}%`,
      });
    }
  ...
  async createVolunteerFromApplicationDto(
    dto: CreateVolunteerFromApplicationDto,
    userId?: number,
  ): Promise<Volunteer> {
    ...
    if (application.status !== ApplicationStatus.APPROVED) {
      throw new BadRequestException(
        'Can only create volunteer from approved application',
      );
    }
    ...
    if (existing) {
      throw new BadRequestException(
        'Volunteer profile already exists for this application',
      );
    }
