...
export class VolunteersController {
    ...
    async getApplicationsLegacy(@Query() query: ApplicationQueryDto, @Request() req: any) {
    ...
    async getApplication(@Param('id') id: string) {
        const application = await this.volunteersService.getApplicationById(parseInt(id, 10));
    ...
    async updateApplicationStatusLegacy(
        ...
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins can approve/reject applications' };
        }
        const application = await this.volunteersService.updateApplicationStatus(
            parseInt(id, 10),
            dto,
    ...
    async bulkUpdateStatus(@Body() dto: BulkStatusUpdateDto, @Request() req: any) {
        const result = await this.volunteersService.bulkUpdateStatus(dto, req.user.id);
        return {
    ...
    async bulkUpdateStatusLegacy(@Body() dto: BulkStatusUpdateDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
    ...
    async exportApplications(
        ...
        const result = await this.volunteersService.exportApplications(format || 'csv', query);
    ...
    async getVolunteer(@Param('id') id: string) {
        const volunteer = await this.volunteersService.getVolunteerById(parseInt(id, 10));
        return { data: volunteer };
    }
    ...
    async createVolunteer(@Body() dto: CreateVolunteerDto, @Request() req: any) {
        const volunteer = await this.volunteersService.createVolunteer(dto, req.user?.id);
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post('admin/volunteers')
    async createVolunteerLegacy(@Body() dto: CreateVolunteerDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
    ...
    async createVolunteerFromApplication(
        ...
        const volunteer = await this.volunteersService.createVolunteerFromApplicationDto(
    ...
    async createVolunteerFromApplicationLegacy(
        ...
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins can create volunteers from applications' };
        }
        const volunteer = await this.volunteersService.createVolunteerFromApplicationDto(
            { ...dto, applicationId: parseInt(id, 10) },
            req.user?.id
    ...
    async updateVolunteer(
        ...
        const volunteer = await this.volunteersService.updateVolunteer(parseInt(id, 10), dto, req.user?.id);
        return { success: true, data: volunteer };
    }
    ...
    async bulkUpdateVolunteerStatus(@Body() dto: BulkVolunteerStatusDto, @Request() req: any) {
        const result = await this.volunteersService.bulkUpdateVolunteerStatus(dto, req.user?.id);
        return {
            success: result.success,
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post('admin/volunteers/bulk-status')
    async bulkUpdateVolunteerStatusLegacy(@Body() dto: BulkVolunteerStatusDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins can perform bulk actions' };
        }
        const result = await this.volunteersService.bulkUpdateVolunteerStatus(dto, req.user?.id);
    ...
    async exportVolunteers(
        ...
        const result = await this.volunteersService.exportVolunteers(format || 'csv', query);
