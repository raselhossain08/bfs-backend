...
export class SavedCampaignsController {
  ...
  async getSavedCampaigns(
    ...
    const savedCampaigns =
      await this.savedCampaignsService.getUserSavedCampaigns(
        req.user.id,
  ...
  async getSavedCount(@Request() req: any): Promise<{ count: number }> {
    const count = await this.savedCampaignsService.getSavedCampaignsCount(
      req.user.id,
    );
    return { count };
  }
  ...
  async saveCampaign(
    ...
    const savedCampaign = await this.savedCampaignsService.saveCampaign(
      req.user.id,
      dto,
    );
