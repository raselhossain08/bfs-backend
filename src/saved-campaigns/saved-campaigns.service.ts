...
export class SavedCampaignsService {
    ...
    async saveCampaign(userId: number, dto: SaveCampaignDto): Promise<SavedCampaign> {
        // Check if cause exists
        const cause = await this.causeRepository.findOne({ where: { id: dto.causeId } });
        if (!cause) {
    ...
    async getUserSavedCampaigns(userId: number, options?: { folder?: string }): Promise<SavedCampaign[]> {
    ...
    async updateSavedCampaign(userId: number, causeId: number, dto: UpdateSavedCampaignDto): Promise<SavedCampaign> {
    ...
    async removeSavedCampaign(userId: number, causeId: number): Promise<{ success: boolean }> {
        const result = await this.savedCampaignRepository.delete({ userId, causeId });
    ...
    async bulkRemove(userId: number, causeIds: number[]): Promise<{ success: boolean; count: number }> {
    ...
    async getCampaignsNearGoal(userId: number, threshold: number = 80): Promise<SavedCampaign[]> {
        ...
        return savedCampaigns.filter((sc) => {
            ...
            const progress = sc.cause.goal > 0 ? (sc.cause.raised / sc.cause.goal) * 100 : 0;
