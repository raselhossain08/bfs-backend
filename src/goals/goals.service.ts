...
export class GoalsService {
  ...
  async getGoalStats(userId: number): Promise<{
    ...
    const completedGoals = goals.filter(
      (g) =>
        parseFloat(g.current.toString()) >= parseFloat(g.target.toString()),
    ).length;
    const totalTarget = goals.reduce(
      (sum, g) => sum + parseFloat(g.target.toString()),
      0,
    );
    const totalCurrent = goals.reduce(
      (sum, g) => sum + parseFloat(g.current.toString()),
      0,
    );
  ...
  async getAggregatedGoals(): Promise<{
    ...
    const monthlyDonationGoal = monthlyGoals.reduce(
      (sum, g) => sum + parseFloat(g.target.toString()),
      0,
    );
    const currentMonthDonations = monthlyGoals.reduce(
      (sum, g) => sum + parseFloat(g.current.toString()),
      0,
    );

    // Count unique causes supported
    const causesWithGoals = new Set(
      allGoals.filter((g) => g.causeId).map((g) => g.causeId),
    );
    ...
    const volunteerGoals = allGoals.filter(
      (g) =>
        g.title.toLowerCase().includes('volunteer') ||
        g.title.toLowerCase().includes('hour'),
    );
    const volunteerHoursGoal = volunteerGoals.reduce(
      (sum, g) => sum + parseFloat(g.target.toString()),
      0,
    );
    const volunteerHours = volunteerGoals.reduce(
      (sum, g) => sum + parseFloat(g.current.toString()),
      0,
    );
