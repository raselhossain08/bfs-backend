...
export class RecurringDonationsController {
    ...
    async findOne(@Request() req: any, @Param('id') id: string) {
        const userId = req.user?.userId || req.user?.sub || req.user?.id;
        const donation = await this.recurringDonationsService.findOne(userId, parseInt(id, 10));
    ...
    async update(
        ...
        const userId = req.user?.userId || req.user?.sub || req.user?.id;
        const donation = await this.recurringDonationsService.update(userId, parseInt(id, 10), dto);
        return { success: true, data: donation };
    }
    ...
    async pause(@Request() req: any, @Param('id') id: string) {
        const userId = req.user?.userId || req.user?.sub || req.user?.id;
        const donation = await this.recurringDonationsService.pause(userId, parseInt(id, 10));
        return { success: true, data: donation };
    }
    ...
    async resume(@Request() req: any, @Param('id') id: string) {
        const userId = req.user?.userId || req.user?.sub || req.user?.id;
        const donation = await this.recurringDonationsService.resume(userId, parseInt(id, 10));
        return { success: true, data: donation };
    }
    ...
    async cancel(@Request() req: any, @Param('id') id: string) {
        const userId = req.user?.userId || req.user?.sub || req.user?.id;
        const result = await this.recurringDonationsService.cancel(userId, parseInt(id, 10));
        return result;
    }
}
