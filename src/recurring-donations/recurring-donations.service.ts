...
export class RecurringDonationsService {
     ...
     */
    private getStripeInterval(frequency: string): { interval: 'week' | 'month' | 'year'; interval_count: number } {
        switch (frequency) {
            case 'weekly':
    ...
    async findOne(userId: number, id: number): Promise<RecurringDonation> {
        ...
        if (donation.userId !== userId) {
            throw new ForbiddenException('You do not have access to this recurring donation');
        }
     ...
     */
    async create(userId: number, dto: CreateRecurringDonationDto): Promise<RecurringDonation> {
        // Get user
        ...
        if (!paymentMethod || paymentMethod.userId !== userId) {
            throw new ForbiddenException('Payment method not found or does not belong to user');
        }

        // Get cause if specified
        let cause: Cause | null = null;
        if (dto.causeId) {
            cause = await this.causeRepository.findOne({ where: { id: dto.causeId } });
            if (!cause) {
        ...
        const saved = await this.recurringDonationRepository.save(recurringDonation);

        this.logger.log(`Created recurring donation ${saved.id} with Stripe subscription ${subscription.id}`);
     ...
     */
    async update(userId: number, id: number, dto: UpdateRecurringDonationDto): Promise<RecurringDonation> {
        const donation = await this.findOne(userId, id);

        // If changing payment method, verify ownership
        if (dto.paymentMethodId && dto.paymentMethodId !== donation.paymentMethodId) {
            const paymentMethod = await this.paymentMethodRepository.findOne({
                where: { id: dto.paymentMethodId },
            });
            ...
            if (donation.stripeSubscriptionId) {
                await this.ensureStripe().subscriptions.update(donation.stripeSubscriptionId, {
                    default_payment_method: paymentMethod.stripePaymentMethodId,
                });
            }
        ...
        if (dto.amount || dto.frequency) {
            ...
            const cause = donation.causeId
                ? await this.causeRepository.findOne({ where: { id: donation.causeId } })
                : null;
            ...
            if (donation.stripeSubscriptionId) {
                const subscription = await this.ensureStripe().subscriptions.retrieve(donation.stripeSubscriptionId);
                const oldPriceId = subscription.items.data[0]?.price.id;

                await this.ensureStripe().subscriptions.update(donation.stripeSubscriptionId, {
                    items: [{ id: subscription.items.data[0].id, price: priceId }],
                    default_payment_method: donation.paymentMethodId
                        ? (await this.paymentMethodRepository.findOne({ where: { id: donation.paymentMethodId } }))?.stripePaymentMethodId
                        : undefined,
                });

                // Archive old price
                if (oldPriceId) {
                    await this.ensureStripe().prices.update(oldPriceId, { active: false }).catch(() => {});
                }
            }
    ...
    async resume(userId: number, id: number): Promise<RecurringDonation> {
        ...
        donation.nextDonationDate = this.calculateNextDonationDate(donation.frequency);
        const saved = await this.recurringDonationRepository.save(donation);
     ...
     */
    async cancel(userId: number, id: number): Promise<{ success: boolean; message: string }> {
        const donation = await this.findOne(userId, id);

        // Cancel Stripe subscription
        if (donation.stripeSubscriptionId) {
            await this.ensureStripe().subscriptions.cancel(donation.stripeSubscriptionId);
        }
     ...
     */
    async handlePaymentSuccess(stripeSubscriptionId: string, amount: number): Promise<void> {
        const donation = await this.recurringDonationRepository.findOne({
            where: { stripeSubscriptionId },
        });

        if (!donation) {
            this.logger.warn(`No recurring donation found for subscription ${stripeSubscriptionId}`);
            return;
        }
        ...
        await this.recurringDonationRepository.save(donation);
        this.logger.log(`Updated donation stats for recurring donation ${donation.id}`);
    }
    ...
    async handleSubscriptionDeleted(stripeSubscriptionId: string): Promise<void> {
        ...
        if (donation) {
            donation.status = 'cancelled';
            await this.recurringDonationRepository.save(donation);
            this.logger.log(`Marked recurring donation ${donation.id} as cancelled via webhook`);
        }
    }
    ...
    async getUserRecurringStats(userId: number): Promise<{
        ...
        const monthlyAmount = activeDonations.reduce((total, d) => {
            const multiplier = d.frequency === 'weekly' ? 4.33
                : d.frequency === 'monthly' ? 1
                : d.frequency === 'quarterly' ? 0.33
                : d.frequency === 'yearly' ? 0.083
                : 1;
