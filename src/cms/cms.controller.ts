import { Controller, Get, Patch, Post, Delete, Param, Body, NotFoundException, UseInterceptors, UploadedFile, Res, UseGuards, Query, Logger, Request } from '@nestjs/common';
...
export class CmsController {
    ...
    @UseInterceptors(FileInterceptor('file', {
        ...
        fileFilter: (req, file, cb) => {
            ...
            const allowed = /\.(jpg|jpeg|png|gif|webp|svg|mp4|webm|mov|mp3|wav|pdf|doc|docx|xls|xlsx|txt)$/i;
    ...
    async uploadFile(@UploadedFile() file: any) {
        ...
        const apiUrl = this.configService.get<string>('API_URL') || 'http://localhost:5000';
    ...
    async deleteBatchUploads(@Body() body: { filenames: string[] }) {
        ...
        this.logger.log(`Batch delete: ${deletedCount}/${filenames.length} files deleted`);
    ...
    async submitContact(@Body() body: any) {
        ...
        if (Array.isArray(contacts)) {
            await this.cmsService.updateData('contactMessages', [...contacts, newContact]);
        } else {
        ...
        await this.emailService.sendContactNotification(adminEmail, { name, email, service, message });
    ...
    async submitVolunteerApplication(@Body() body: any) {
        const {
            name, email, phone, interest, location,
            skills, experience, availability, preferredContact,
            emergencyContactName, emergencyContactPhone,
            languages, hasVolunteeredBefore, previousVolunteerDetails,
            message, causeId, causeTitle
        } = body;

        if (!name || !email || !phone || !interest || !location) {
            throw new NotFoundException('Name, email, phone, interest and location are required');
        }

        // Save volunteer application to CMS
        const applications = await this.cmsService.getData('volunteerApplications') || [];
        ...
        if (Array.isArray(applications)) {
            await this.cmsService.updateData('volunteerApplications', [...applications, newApplication]);
        } else {
            await this.cmsService.updateData('volunteerApplications', [newApplication]);
        }
        ...
        this.logger.log(`Volunteer application submitted by ${name} (${email}) for ${causeTitle || 'General'}`);
    ...
    async submitEventRegistration(@Body() body: any) {
        const { name, email, phone, organization, eventId, eventTitle, message } = body;
        ...
        const registrations = await this.cmsService.getData('eventRegistrations') || [];
        ...
        if (Array.isArray(registrations)) {
            await this.cmsService.updateData('eventRegistrations', [...registrations, newRegistration]);
        } else {
            await this.cmsService.updateData('eventRegistrations', [newRegistration]);
        }

        this.logger.log(`Event registration submitted by ${name} (${email}) for ${eventTitle || 'General Event'}`);

        return {
            success: true,
            message: 'Registration successful! We will contact you with event details.',
    ...
    async resendWelcomeEmail(@Body() body: any) {
        ...
        if (sent) {
            ...
            return { data: { success: true, message: 'Welcome email sent successfully' } };
        } else {
    ...
    async migrateSubscribers() {
        ...
        const subscribersKeyData = await this.cmsService.getData('subscribers') || [];

        if (!subscribeKeyData || !Array.isArray(subscribeKeyData) || subscribeKeyData.length === 0) {
            return {
                success: false,
                message: 'No legacy data to migrate',
                data: {
                    legacyCount: 0,
                    currentCount: Array.isArray(subscribersKeyData) ? subscribersKeyData.length : 0
                }
            };
        }
    ...
    async getSubscriberStats() {
        ...
        // Get last sent date
        const sentNewsletters = newsletterList.filter((n: any) => n.status === 'sent');
        const lastSent = sentNewsletters.length > 0
            ? sentNewsletters.sort((a: any, b: any) =>
                new Date(b.sentAt).getTime() - new Date(a.sentAt).getTime())[0]
            : null;
        ...
        return {
            data: {
                ...
                totalRecipients: sentNewsletters.reduce((sum: number, n: any) => sum + (n.recipientCount || 0), 0),
            }
        };
    }
    ...
    async subscribe(@Body() body: any) {
        ...
        if (Array.isArray(subscribers) && subscribers.some((s: any) =>
            s.email?.toLowerCase() === normalizedEmail
        )) {
        ...
        return { data: { success: true, message: 'Subscribed successfully', subscriber: newSubscriber } };
    }
    ...
    async updateSmsSettings(@Body() body: any) {
        const existingSettings = await this.cmsService.getData('smsSettings') || {};
        ...
        const updated = await this.cmsService.updateData('smsSettings', {
            twilioAccountSid: body.twilioAccountSid || existingSettings.twilioAccountSid || '',
            twilioAuthToken: authToken || existingSettings.twilioAuthToken || '',
            twilioPhoneNumber: body.twilioPhoneNumber || existingSettings.twilioPhoneNumber || '',
    ...
    async testSmsConnection() {
        ...
        if (!settings.twilioAccountSid || !settings.twilioAuthToken || !settings.twilioPhoneNumber) {
            return {
                success: false,
                error: 'SMS credentials not configured. Please add your Twilio credentials.',
            };
        }
        ...
        try {
            ...
            const account = await client.api.accounts(settings.twilioAccountSid).fetch();

            this.logger.log(`SMS connection test successful for account: ${account.friendlyName}`);
    ...
    async broadcastAlert(@Body() body: { message: string; audience?: string[] }, @Request() req: any) {
        ...
        if (audience.includes('volunteers') || audience.includes('all')) {
            const volunteers = await this.cmsService.getData('volunteerApplications') || [];
        ...
        if (audience.includes('events') || audience.includes('all')) {
            const eventRegistrations = await this.cmsService.getData('eventRegistrations') || [];
        ...
        const uniqueRecipients = new Map<string, { phone: string; name: string; type: string }>();
        ...
        if (phoneNumbers.length > 0) {
            if (smsReady) {
                ...
                this.logger.log(`[DEV MODE] Would send alert to ${phoneNumbers.length} recipients:`);
        ...
        await this.cmsService.updateData('alertBroadcasts', [...(Array.isArray(broadcasts) ? broadcasts : []), newBroadcast]);

        this.logger.log(`Alert broadcast: "${message.substring(0, 50)}..." sent to ${smsResult.success}/${recipientList.length} recipients`);
    ...
    async getBroadcastStats() {
        ...
        return {
            data: {
                ...
                lastBroadcast: Array.isArray(broadcasts) && broadcasts.length > 0
    ...
    async updateEmailSettings(@Body() body: any) {
        const existingSettings = await this.cmsService.getData('emailSettings') || {};
    ...
    async testEmailConnection(@Request() req: any) {
        ...
        if (!settings.smtpHost || !settings.smtpUser || !settings.smtpPass) {
            return {
                success: false,
                error: 'Email settings not configured. Please add your SMTP credentials.',
            };
        }
        ...
        try {
            ...
            this.logger.log(`Email connection test successful for: ${settings.smtpUser}`);
    ...
    async updateSeoSettings(@Body() body: any) {
        const existingSettings = await this.cmsService.getData('seoSettings') || {};

        const updated = await this.cmsService.updateData('seoSettings', {
            metaTitle: body.metaTitle !== undefined ? body.metaTitle : existingSettings.metaTitle || '',
            metaDescription: body.metaDescription !== undefined ? body.metaDescription : existingSettings.metaDescription || '',
            metaKeywords: body.metaKeywords !== undefined ? body.metaKeywords : existingSettings.metaKeywords || '',
            ogImage: body.ogImage !== undefined ? body.ogImage : existingSettings.ogImage || null,
            twitterCard: body.twitterCard !== undefined ? body.twitterCard : existingSettings.twitterCard || 'summary_large_image',
            googleAnalyticsId: body.googleAnalyticsId !== undefined ? body.googleAnalyticsId : existingSettings.googleAnalyticsId || '',
            enableSitemap: body.enableSitemap !== undefined ? body.enableSitemap : existingSettings.enableSitemap ?? true,
            enableRobots: body.enableRobots !== undefined ? body.enableRobots : existingSettings.enableRobots ?? true,
        });
    ...
    async importSettings(@Body() body: any) {
        ...
        try {
            ...
            if (data.contactSettings && !options.skipContact) {
                await this.cmsService.updateData('contactSettings', data.contactSettings);
            }
    ...
    async resetSettings() {
        const defaultSettings = {
            global: {
                ...
                siteDescription: 'Building a better future through community support and sustainable development.',
                logo: null,
                favicon: null,
                primaryColor: '#14b8a6',
        ...
        try {
            ...
            await this.cmsService.updateData('contactSettings', defaultSettings.contactSettings);
            await this.cmsService.updateData('smsSettings', defaultSettings.smsSettings);
            await this.cmsService.updateData('emailSettings', defaultSettings.emailSettings);
            await this.cmsService.updateData('seoSettings', defaultSettings.seoSettings);
    ...
    async getMediaStats() {
        ...
        const documents = items.filter((item: any) => item.type === 'document').length;
    ...
    async bulkDeleteDonations(@Body() body: { ids: string[] }) {
        ...
        return { success: true, message: `${deletedCount} donations deleted`, deletedCount };
    }
    ...
    async addItem(@Param('key') key: string, @Body() body: any) {
        ...
        if (!result) {
            throw new NotFoundException(`Key "${key}" is not an array or could not be found`);
        }
        return { data: result };
    }
    ...
    async getData(@Param('key') key: string, @Query() query: any) {
        let data = await this.cmsService.getData(key);

        if (!data) {
            // Return empty array for array-type keys
            if (key.startsWith('comments-') || key === 'subscribers' || key === 'contactMessages' || key === 'reports' || key === 'admins' || key === 'teamMembers') {
                return { data: [] };
            }
    ...
    async broadcastNewsletter(@Body() body: any) {
        ...
        if (Array.isArray(newsletters)) {
            await this.cmsService.updateData('newsletters', [...newsletters, newNewsletter]);
        } else {
            await this.cmsService.updateData('newsletters', [newNewsletter]);
        }

        this.logger.log(`Newsletter broadcast: "${subject}" sent to ${result.success} subscribers`);
    ...
    async generateReport(@Body() body: { type: string; title?: string; description?: string; startDate?: string; endDate?: string }) {
        ...
        const validTypes = ['donations', 'volunteers', 'events', 'subscribers', 'contacts', 'campaigns'];
        if (!validTypes.includes(type)) {
            throw new NotFoundException(`Invalid report type. Valid types: ${validTypes.join(', ')}`);
        }
    ...
    async downloadReport(@Param('id') id: string, @Res() res: Response) {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${report.filename || `report-${id}.csv`}"`);
    ...
    async submitArticleComment(@Body() body: any) {
        ...
        this.logger.log(`New comment submitted by ${name} on article: ${articleTitle}`);
    ...
    async getArticleCommentsBySlug(@Param('slug') slug: string) {
        ...
        const articleComments = Array.isArray(comments)
            ? comments.filter((c: any) => c.articleSlug === slug && c.status === 'approved')
            : [];
