...
export class CategoriesController {
    ...
    async getAdminCategoriesLegacy(@Query() query: CategoryQueryDto, @Request() req: any) {
    ...
    async createCategoryLegacy(@Body() dto: CreateCategoryDto, @Request() req: any) {
    ...
    async bulkUpdateStatusLegacy(@Body() dto: BulkStatusDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
    ...
    async exportCategories(
        ...
        // CSV format
        const headers = ['ID', 'Name', 'Slug', 'Description', 'Status', 'Order', 'Parent ID', 'Created At'];
