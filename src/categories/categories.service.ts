...
export class CategoriesService {
  ...
  async create(dto: CreateCategoryDto): Promise<Category> {
    ...
    const categoryData = {
      ...dto,
      metaTitle:
        dto.metaTitle || this.generateMetaTitle(dto.name, dto.parentId),
      metaDescription:
        dto.metaDescription ||
        this.generateMetaDescription(dto.description, dto.name),
      keywords:
        dto.keywords || this.generateKeywords(dto.name, dto.description),
  ...
  async reorder(dto: ReorderCategoriesDto): Promise<{ success: boolean }> {
    if (dto.ids.length !== dto.orders.length) {
      throw new BadRequestException(
        'ids and orders arrays must have the same length',
      );
    }
  ...
  async bulkUpdateStatus(
    ...
    this.logger.log(
      `Bulk status update: ${result.affected} categories set to ${dto.status}`,
    );
