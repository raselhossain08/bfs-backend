...
import { SendInvitesDto, ReferralStatsDto, ReferralDto } from './dto/referral.dto';
...
export class ReferralService {
     ...
     */
    async processReferral(newUserId: number, referralCode: string, email: string): Promise<Referral | null> {
        ...
        this.logger.log(`Processed referral: User ${newUserId} referred by ${referrer.id}`);
    ...
    async recordDonation(userId: number, amount: number): Promise<void> {
        ...
        referral.totalDonated = parseFloat(referral.totalDonated.toString()) + amount;
        referral.donationCount += 1;
        referral.status = ReferralStatus.DONATED;
        await this.referralRepository.save(referral);

        this.logger.log(`Recorded donation for referral ${referral.id}: $${amount}`);
    }
    ...
    async getStats(userId: number): Promise<ReferralStatsDto> {
        ...
        const joinedReferrals = referrals.filter(
            (r) => r.status === ReferralStatus.JOINED || r.status === ReferralStatus.DONATED,
        ).length;
    ...
    async getReferrals(userId: number): Promise<ReferralDto[]> {
        ...
        return referrals.map((r) => ({
            id: r.id,
            email: r.email,
            name: r.referredUserId && userMap.get(r.referredUserId)
                ? `${userMap.get(r.referredUserId)!.firstName || ''} ${userMap.get(r.referredUserId)!.lastName || ''}`.trim() || r.email
                : r.email,
     ...
     */
    async sendInvites(userId: number, dto: SendInvitesDto): Promise<{ sent: number; failed: number }> {
        const user = await this.userRepository.findOne({ where: { id: userId } });
        if (!user) {
            throw new Error('User not found');
        }
        ...
        const referralCode = await this.getReferralCode(userId);
        const referrerName = `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'A friend';
     ...
     */
    private calculateBadges(totalReferrals: number, totalDonations: number): string[] {
        const badges: string[] = [];
     ...
     */
    async getLeaderboard(limit: number = 10): Promise<Array<{ userId: number; name: string; referrals: number; donations: number }>> {
        ...
        return result.map((r) => ({
            ...
            name: userMap.get(r.userId)
                ? `${userMap.get(r.userId)!.firstName || ''} ${userMap.get(r.userId)!.lastName || ''}`.trim() || 'Anonymous'
                : 'Anonymous',
     ...
     */
    async getAllReferrals(options: { page: number; limit: number }): Promise<{ referrals: Referral[]; total: number }> {
    ...
    async getSystemStats(): Promise<{
        ...
        const totalReferredDonations = parseFloat(totalReferredDonationsResult?.total || '0');
        const averageDonationPerReferral = donatedCount > 0 ? totalReferredDonations / donatedCount : 0;
        ...
        return {
            ...
            donatedCount,
            totalReferredDonations,
            averageDonationPerReferral: Math.round(averageDonationPerReferral * 100) / 100,
        };
    }
}