...
export class ServicesService {
    ...
    async createCategory(dto: CreateServiceCategoryDto): Promise<ServiceCategory> {
        ...
        this.logger.log(`Service category created: ${saved.name} (id: ${saved.id})`);
    ...
    async reorderCategories(dto: ReorderCategoriesDto): Promise<{ success: boolean }> {
    ...
    async findPublicServices(query: { limit?: number; categoryId?: number; isFeatured?: boolean }) {
        ...
        if (isFeatured) {
            queryBuilder.andWhere('service.isFeatured = :isFeatured', { isFeatured: true });
        }
    ...
    async updateService(id: number, dto: UpdateServiceDto): Promise<Service> {
        ...
        if (dto.slug && dto.slug !== service.slug) {
            ...
            if (existing) {
                throw new BadRequestException(`Service with slug '${dto.slug}' already exists`);
            }
        }
    ...
    async bulkUpdateStatus(dto: BulkServiceStatusDto): Promise<{ success: boolean; count: number }> {
        const result = await this.serviceRepository.update(
            { id: In(dto.ids) },
            { status: dto.status }
        );

        this.logger.log(`Bulk status update: ${result.affected} services set to ${dto.status}`);

        return { success: true, count: result.affected || 0 };
    }

    async reorderServices(dto: ReorderServicesDto): Promise<{ success: boolean }> {
        for (const item of dto.orders) {
    ...
    async getServiceStats(): Promise<{
        ...
        const [total, active, inactive, draft, featured, inquiries, views] = await Promise.all([
    ...
    async createInquiry(dto: CreateServiceInquiryDto): Promise<ServiceInquiry> {
        ...
        await this.serviceRepository.increment({ id: dto.serviceId }, 'inquiryCount', 1);

        this.logger.log(`Inquiry created for service ${dto.serviceId}: ${saved.name}`);
    ...
    async updateInquiry(id: number, dto: UpdateServiceInquiryDto): Promise<ServiceInquiry> {
    ...
    async bulkUpdateInquiryStatus(dto: BulkInquiryStatusDto): Promise<{ success: boolean; count: number }> {
        const result = await this.inquiryRepository.update(
            { id: In(dto.ids) },
            { status: dto.status }
        );

        this.logger.log(`Bulk inquiry status update: ${result.affected} inquiries set to ${dto.status}`);
