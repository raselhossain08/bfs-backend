...
export class ServicesController {
    ...
    async getAdminCategoriesLegacy(@Query() query: ServiceCategoryQueryDto, @Request() req: any) {
    ...
    async getCategory(@Param('id') id: string) {
        const category = await this.servicesService.findOneCategory(parseInt(id, 10));
    ...
    async createCategoryLegacy(@Body() dto: CreateServiceCategoryDto, @Request() req: any) {
    ...
    async updateCategory(
        ...
        const category = await this.servicesService.updateCategory(parseInt(id, 10), dto);
        return { success: true, data: category };
    }
    ...
    async reorderCategoriesLegacy(@Body() dto: ReorderCategoriesDto, @Request() req: any) {
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Access denied' };
        }
        await this.servicesService.reorderCategories(dto);
    ...
    @UseGuards(AuthGuard('jwt'))
    @Get('admin/services')
    async getAdminServicesLegacy(@Query() query: ServiceQueryDto, @Request() req: any) {
        if (!ADMIN_ROLES.includes(req.user?.role)) {
            return { data: [], total: 0, message: 'Access denied' };
        }
        return this.servicesService.findAllServices(query);
    ...
    async getServiceStatsLegacy(@Request() req: any) {
        if (!ADMIN_ROLES.includes(req.user?.role)) {
            return { data: { total: 0, active: 0, inactive: 0, draft: 0, featured: 0, totalInquiries: 0, totalViews: 0 } };
        }
        const stats = await this.servicesService.getServiceStats();
        return { data: stats };
    }
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post('admin/services')
    async createServiceLegacy(@Body() dto: CreateServiceDto, @Request() req: any) {
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins and editors can create services' };
        }
        const service = await this.servicesService.createService(dto);
        return { success: true, data: service };
    }
    ...
    async updateService(
        ...
        const service = await this.servicesService.updateService(parseInt(id, 10), dto);
        return { success: true, data: service };
    }
    ...
    async bulkUpdateStatusLegacy(@Body() dto: BulkServiceStatusDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
    ...
    async reorderServicesLegacy(@Body() dto: ReorderServicesDto, @Request() req: any) {
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Access denied' };
        }
        await this.servicesService.reorderServices(dto);
    ...
    async getInquiriesLegacy(@Query() query: ServiceInquiryQueryDto, @Request() req: any) {
        if (!ADMIN_ROLES.includes(req.user?.role)) {
            return { data: [], total: 0, message: 'Access denied' };
        }
        return this.servicesService.findAllInquiries(query);
    ...
    async updateInquiry(
        ...
        const inquiry = await this.servicesService.updateInquiry(parseInt(id, 10), dto);
    ...
    async bulkUpdateInquiryStatusLegacy(@Body() dto: BulkInquiryStatusDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins can perform bulk actions' };
        }
        const result = await this.servicesService.bulkUpdateInquiryStatus(dto);
