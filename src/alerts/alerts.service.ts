...
export class AlertsService {
    ...
    async findTemplateById(id: number): Promise<AlertTemplate> {
        const template = await this.alertTemplateRepository.findOne({ where: { id } });
        if (!template) {
    ...
    async updateTemplate(id: number, dto: UpdateAlertTemplateDto): Promise<AlertTemplate> {
    ...
    // Broadcast methods
    async createBroadcast(dto: BroadcastAlertDto, userId: string): Promise<AlertBroadcast> {
    ...
    async getStats() {
        const [
            ...
            this.alertBroadcastRepository
                .createQueryBuilder('broadcast')
                .where(':audience = ANY(broadcast.audience)', { audience: 'volunteers' })
                .getCount(),
            this.alertBroadcastRepository
                .createQueryBuilder('broadcast')
    ...
    async updateBroadcastStats(id: number, sentCount: number, failedCount: number): Promise<void> {
    ...
    async exportBroadcasts(format: 'csv' | 'json', query: AlertBroadcastQueryDto) {
        ...
        // CSV format
        const headers = ['ID', 'Message', 'Audience', 'Recipients', 'Sent', 'Failed', 'Sent At', 'Sent By'];
