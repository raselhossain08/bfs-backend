...
export class SettingsService {
    ...
    async setSetting(key: string, value: any, category?: string): Promise<Setting> {
    ...
    async getGlobalSettings(): Promise<any> {
        const keys = ['siteName', 'siteDescription', 'logo', 'favicon', 'primaryColor', 'fontFamily', 'enableAnimations', 'maintenanceMode'];
        ...
        return {
            siteName: settings.siteName || 'BFS Foundation',
    ...
    async getContactSettings(): Promise<any> {
        ...
        return {
            ...
            headquarters: settings.headquarters || { line1: '', line2: '', mapsUrl: '' },
            regionalHubs: settings.regionalHubs || [],
            socials: settings.socials || { facebook: '', twitter: '', instagram: '', linkedin: '', youtube: '' },
    ...
    async exportAllSettings(): Promise<any> {
        const settings = await this.settingsRepository.find({ where: { isActive: true } });
        const exported = {};
