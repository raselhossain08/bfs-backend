...
export class ChatService {
  ...
  async updateSessionStatus(sessionId: number, status: SessionStatus) {
    await this.sessionRepo.update(sessionId, {
      status,
      endedAt:
        status === SessionStatus.CLOSED || status === SessionStatus.RESOLVED
          ? new Date()
          : undefined,
    });
  ...
  async processBotMessage(
    sessionId: number,
    userMessage: string,
  ): Promise<{ message: ChatMessageEntity; shouldEscalate: boolean }> {
    const botResponse: BotResponse =
      await this.botService.processMessage(userMessage);
  ...
  async getAdminStats() {
    const [activeSessions, waitingSessions, onlineAgents, unreadMessages] =
      await Promise.all([
