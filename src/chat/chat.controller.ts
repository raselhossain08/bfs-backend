...
export class ChatController {
    ...
    async startSession(@Body() body: { pageUrl?: string; userAgent?: string }) {
        const session = await this.chatService.startSession(
            undefined,
            undefined,
            { pageUrl: body.pageUrl, userAgent: body.userAgent }
        );
        return { data: session };
    }
    ...
    async updateAgentStatus(
        ...
        const agent = await this.chatService.updateAgentStatus(req.user.userId, status);
        return { data: agent };
    }
    ...
    async getAgentSessions(@Request() req) {
        ...
        const agentSessions = sessions.filter(s => s.assignedAgentId === req.user.userId);
    ...
    async getSessionDetails(@Request() req, @Param('id') id: string) {
        ...
        const sessionData = await this.chatService.getSessionWithMessages(parseInt(id, 10));
    ...
    async assignAgent(
        ...
        const session = await this.chatService.assignAgentToSession(sessionId, agentId);
        return { data: session };
    }
    ...
    async updateSessionStatus(
        ...
        const session = await this.chatService.updateSessionStatus(parseInt(id, 10), status);
        return { data: session };
    }
