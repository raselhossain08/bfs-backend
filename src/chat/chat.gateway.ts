...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
        ...
        try {
            ...
            if (token) {
                try {
                    ...
                    console.log(`⚠️ JWT verification failed, treating as anonymous:`, jwtError.message);
                }
            } else {
                ...
                if (sessionToken) {
                    ...
                    console.log(`🔗 Anonymous session: ${sessionToken.substring(0, 8)}...`);
                }
    ...
    async handleSendMessage(
        @ConnectedSocket() client: Socket,
        @MessageBody() data: { sessionId: number; content: string; tempId?: string },
    ) {
        const userId = client.data.user?.userId;
        const sessionToken = client.data.sessionToken;
        ...
        this.server.to(`session:${data.sessionId}`).emit('new_message', userMessage);
        ...
        if (session.isBotActive) {
            ...
            const botResponse = await this.chatService.processBotMessage(data.sessionId, data.content);

            // Broadcast bot response
            this.server.to(`session:${data.sessionId}`).emit('new_message', botResponse.message);

            // If escalation needed, notify admins
            if (botResponse.shouldEscalate) {
                await this.chatService.updateSessionStatus(data.sessionId, SessionStatus.WAITING);
    ...
    async handleAgentUpdateStatus(
        ...
        const agentId = client.data.user?.userId;
        const agent = await this.chatService.updateAgentStatus(agentId, data.status);
    ...
    async handleResolveSession(
        ...
        await this.chatService.updateSessionStatus(data.sessionId, SessionStatus.RESOLVED);
        this.server.to(`session:${data.sessionId}`).emit('session_resolved', { sessionId: data.sessionId });
        this.server.to('admins').emit('admin:session_resolved', { sessionId: data.sessionId });
    ...
    async handleAdminAssignAgent(
        ...
        this.server.to(`session:${data.sessionId}`).emit('agent_joined', agentInfo);
        this.server.to('admins').emit('admin:agent_assigned', { sessionId: data.sessionId, agent: agentInfo });
