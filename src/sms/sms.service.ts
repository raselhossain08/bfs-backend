...
export class SmsService {
    ...
    private async getCredentials(): Promise<{
        ...
        try {
            ...
            if (settings.twilioAccountSid && settings.twilioAuthToken && settings.smsEnabled) {
                return {
            ...
            this.logger.warn('Could not fetch SMS settings from database, using environment variables');
        }
    ...
    async sendSms(to: string, message: string): Promise<SmsResult> {
        ...
        if (!credentials.enabled || !credentials.accountSid || !credentials.authToken) {
            // Development mode: log the SMS instead of sending
            this.logger.log(`[DEV MODE] SMS would be sent to ${formattedPhone}: ${message.substring(0, 50)}...`);
            return {
                success: true,
        ...
        try {
            ...
            this.logger.log(`SMS sent successfully to ${formattedPhone}: ${result.sid}`);
    ...
    async sendBulkSms(recipients: string[], message: string): Promise<BulkSmsResult> {
        ...
        this.logger.log(`Bulk SMS complete: ${successCount} sent, ${failedCount} failed`);
