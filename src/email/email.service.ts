...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      auth: {
        user:
          this.configService.get<string>('SMTP_USER') || 'info@birdsfly.org',
  ...
  async sendEmail(to: string, subject: string, html: string): Promise<boolean> {
    try {
      const mailOptions: nodemailer.SendMailOptions = {
        from: `"Birdsfly Sangstha" <${this.emailFrom}>`,
        to,
        subject,
        html,
      };
  ...
  async sendNewsletterBroadcast(
    ...
    const baseUrl =
      this.configService.get<string>('FRONTEND_URL') || 'https://birdsfly.org';
  ...
  async sendBulkNewsletter(
    ...
    const results = await Promise.allSettled(
      recipients.map((email) =>
        this.sendNewsletterBroadcast(email, subject, content),
      ),
    );
    ...
    this.logger.log(
      `Newsletter broadcast complete: ${success} sent, ${failed} failed`,
    );
  ...
  async sendVolunteerApplicationConfirmation(
    ...
    const subject = EmailTemplates.getVolunteerApplicationConfirmationSubject(
      data.causeTitle,
    );
    return this.sendEmail(to, subject, html);
  }

  async sendVolunteerApplicationAdminNotification(data: {
    name: string;
    email: string;
    phone: string;
    interest: string;
    ...
    const html =
      EmailTemplates.getVolunteerApplicationAdminNotificationHTML(data);
    const subject = EmailTemplates.getVolunteerApplicationAdminSubject(
      data.causeTitle,
    );
