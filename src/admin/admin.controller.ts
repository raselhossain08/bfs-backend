...
export class AdminController {
    ...
    async getStats(@Request() req: any) {
        if (!this.isAdmin(req.user)) {
            return { data: { total: 0, superAdmins: 0, admins: 0, editors: 0, managers: 0, donors: 0, active: 0 } };
        }
    ...
    async exportUsers(
        ...
        if (!this.isSuperAdmin(req.user)) {
            return res.status(403).json({ success: false, message: 'Only super admins can export users' });
        }

        const result = await this.adminService.exportUsers(format, req.user.id, req);

        if (format === 'csv') {
            // Convert to CSV
            const headers = ['ID', 'First Name', 'Last Name', 'Email', 'Role', 'Status', 'Phone', 'Last Active', 'Created At'];
    ...
    async update(
        ...
        const user = await this.adminService.update(parseInt(id, 10), dto, req.user.id, req);
        return {
            success: true,
    ...
    async updateRole(
        ...
        const user = await this.adminService.updateRole(parseInt(id, 10), dto, req.user.id, req);
        return {
            success: true,
    ...
    async updateStatus(
        ...
        const user = await this.adminService.updateStatus(parseInt(id, 10), dto, req.user.id, req);
        return {
            success: true,
    ...
    async initiatePasswordReset(
        ...
        const result = await this.adminService.initiatePasswordReset(parseInt(id, 10), req.user.id, req);
        return result;
    }
    ...
    async bulkUpdateStatus(
        ...
        const result = await this.adminService.bulkUpdateStatus(dto.ids, dto.value, req.user.id, req);
        return {
    ...
    async bulkUpdateRole(
        ...
        const result = await this.adminService.bulkUpdateRole(dto.ids, dto.value, req.user.id, req);
        return {
            success: result.success,
    ...
    async bulkDelete(
        ...
        const result = await this.adminService.bulkDelete(dto.ids, req.user.id, req);
        return {
            success: result.success,
