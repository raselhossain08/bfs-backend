...
import { CreateAdminDto, UpdateAdminDto, UpdateRoleDto, UpdateStatusDto, AdminListQueryDto, UserRole } from './admin.dto';
...
export class AdminService {
    ...
    private readonly adminRoles = [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.EDITOR, UserRole.MANAGER];
    ...
    private sanitizeUser(user: User) {
        const { password, resetToken, resetTokenExpiry, refreshToken, otpLoginCode, otpLoginExpiry, otpAttemptsResetAt, twoFactorSecret, twoFactorBackupCodes, ...safe } = user;
        return safe;
    }
    ...
    async create(dto: CreateAdminDto, actorId: number, request?: Request): Promise<any> {
        ...
        const avatar = dto.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName)}&background=e11d48&color=fff&bold=true`;
        ...
        await this.auditService.log({
            ...
            actorId,
            newValues: { email: savedUser.email, role: savedUser.role, status: savedUser.status },
    ...
    async update(id: number, dto: UpdateAdminDto, actorId: number, request?: Request): Promise<any> {
        const user = await this.usersRepository.findOne({ where: { id } });
        if (!user) {
            throw new NotFoundException('User not found');
        }
        ...
        await this.auditService.log({
            ...
            entityId: savedUser.id,
            actorId,
            oldValues: { firstName: oldValues.firstName, lastName: oldValues.lastName, email: oldValues.email, role: oldValues.role },
            newValues: { firstName: savedUser.firstName, lastName: savedUser.lastName, email: savedUser.email, role: savedUser.role },
    ...
    async updateRole(id: number, dto: UpdateRoleDto, actorId: number, request?: Request): Promise<any> {
        const user = await this.usersRepository.findOne({ where: { id } });
        if (!user) {
            throw new NotFoundException('User not found');
        }
    ...
    async updateStatus(id: number, dto: UpdateStatusDto, actorId: number, request?: Request): Promise<any> {
        const user = await this.usersRepository.findOne({ where: { id } });
        if (!user) {
            throw new NotFoundException('User not found');
        }
    ...
    async remove(id: number, actorId: number, request?: Request): Promise<{ message: string }> {
        const user = await this.usersRepository.findOne({ where: { id } });
        if (!user) {
            throw new NotFoundException('User not found');
        }
    ...
    // Bulk operations
    async bulkUpdateStatus(ids: number[], status: string, actorId: number, request?: Request): Promise<{ success: boolean; count: number }> {
    ...
    async bulkUpdateRole(ids: number[], role: string, actorId: number, request?: Request): Promise<{ success: boolean; count: number }> {
    ...
    async bulkDelete(ids: number[], actorId: number, request?: Request): Promise<{ success: boolean; count: number }> {
        const users = await this.usersRepository.find({ where: { id: In(ids) } });
    ...
    // Password reset
    async initiatePasswordReset(id: number, actorId: number, request?: Request): Promise<{ success: boolean; message: string }> {
        const user = await this.usersRepository.findOne({ where: { id } });
        if (!user) {
            throw new NotFoundException('User not found');
        }
        ...
        const resetToken = bcrypt.genSaltSync(32).replace(/[./]/g, '').substring(0, 32);
    ...
    // Export users
    async exportUsers(format: 'csv' | 'json', actorId: number, request?: Request): Promise<{ data: User[]; format: string }> {
        const users = await this.usersRepository.find({
            where: { role: In(this.adminRoles) },
            order: { createdAt: 'DESC' },
            select: ['id', 'firstName', 'lastName', 'email', 'role', 'status', 'phone', 'lastActive', 'createdAt'],
        });
    ...
    async getStats(): Promise<{
        ...
        const [total, superAdmins, admins, editors, managers, donors, active] = await Promise.all([
