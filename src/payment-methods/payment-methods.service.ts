...
export class PaymentMethodsService {
    ...
    async create(userId: number, dto: CreatePaymentMethodDto): Promise<PaymentMethod> {
        ...
        const stripePM = await this.stripe.paymentMethods.retrieve(dto.paymentMethodId);
    ...
    async remove(userId: number, id: number): Promise<{ success: boolean }> {
        ...
        try {
            ...
            this.logger.warn(`Failed to detach payment method from Stripe: ${error.message}`);
    ...
    async syncFromStripe(userId: number): Promise<PaymentMethod[]> {
        ...
        const existingIds = new Set(existingPMs.map(pm => pm.stripePaymentMethodId));
        ...
        for (const existingPM of existingPMs) {
            const stillExists = stripePMs.data.some(sp => sp.id === existingPM.stripePaymentMethodId);
            if (!stillExists) {
