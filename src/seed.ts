...
async function seed() {
    ...
    try {
        ...
        const existingAdmin = await userRepository.findOne({ where: { email: 'admin@birdsfly.org' } });
        ...
        if (fs.existsSync(dataPath)) {
            ...
            const preserveKeys = ['subscribers', 'contactMessages', 'newsletters', 'dashboard-activities'];

            let migratedCount = 0;
            for (const [key, value] of Object.entries(cmsData)) {
                let existingItem = await cmsRepository.findOne({ where: { key } });
                if (existingItem) {
            ...
            console.log(`Successfully migrated ${migratedCount} CMS top-level keys into the database.`);
        } else {
