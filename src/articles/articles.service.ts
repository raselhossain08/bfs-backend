...
export class ArticlesService {
  ...
  private transformArticle(article: Article) {
    ...
    const parsedContentBlocks =
      typeof article.contentBlocks === 'string'
        ? (() => {
            try {
              return JSON.parse(article.contentBlocks);
            } catch {
              return [];
            }
          })()
  ...
  async findPublic(query: PublicArticleQueryDto) {
    ...
    queryBuilder.orderBy(`article.${sortBy}`, sortOrder).skip(skip).take(limit);
  ...
  async bulkUpdateStatus(
    ...
    this.logger.log(
      `Bulk status update: ${result.affected} articles set to ${dto.status}`,
    );
  ...
  async getStats(): Promise<{
    ...
    const [total, published, draft, archived, viewsResult, likesResult] =
      await Promise.all([
