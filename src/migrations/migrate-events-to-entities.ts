...
async function createDefaultEventTypes() {
    ...
    // Default event types
    const defaultTypes = [
        { name: 'Mission', slug: 'mission', description: 'Field missions and outreach programs', color: '#0d9488', order: 1 },
        { name: 'Fundraiser', slug: 'fundraiser', description: 'Fundraising events and campaigns', color: '#f59e0b', order: 2 },
        { name: 'Community', slug: 'community', description: 'Community engagement events', color: '#10b981', order: 3 },
        { name: 'Workshop', slug: 'workshop', description: 'Educational workshops and training sessions', color: '#6366f1', order: 4 },
        { name: 'Conference', slug: 'conference', description: 'Conferences and large gatherings', color: '#8b5cf6', order: 5 },
        { name: 'Other', slug: 'other', description: 'Other events', color: '#64748b', order: 6 },
    ];
    ...
    for (const type of defaultTypes) {
        await dataSource.query(
            ...
            [type.name, type.slug, type.description, type.color, type.order, 'active']
        );
...
async function migrateEvents() {
    ...
    for (const oldEvent of oldEvents) {
        ...
        // Get event type ID
        const eventTypeId = oldEvent.category ? eventTypeNameToId.get(oldEvent.category) : null;
        ...
        try {
            await dataSource.query(
                ...
                [
                    ...
                    oldEvent.contentBlocks ? JSON.stringify(oldEvent.contentBlocks) : null,
