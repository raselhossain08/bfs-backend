...
async function createDefaultCategories() {
  ...
  const defaultCategories = [
    ...
    },
    {
      ...
      description: 'Climate adaptation and environmental programs',
      color: '#22c55e',
      order: 3,
    },
    {
      name: 'Clean Water',
      slug: 'clean-water',
      description: 'Clean water and sanitation programs',
      color: '#0ea5e9',
      order: 4,
    },
    {
    ...
    },
    {
      ...
      description: 'Women empowerment and gender equality',
      color: '#ec4899',
      order: 7,
    },
    {
  ...
  for (const cat of categorySet) {
    const exists = defaultCategories.find(
      (d) =>
        d.name.toLowerCase() === cat.toLowerCase() ||
        d.slug === generateSlug(cat),
    );
    if (!exists) {
...
async function migrateCauses() {
  ...
  const existingCount = await dataSource.query(`SELECT COUNT(*) FROM cause`);
  ...
  const categories = await dataSource.query(
    `SELECT id, name, slug FROM cause_category`,
  );
  ...
  for (const oldCause of oldCauses) {
    ...
    if (oldCause.category) {
      categoryId =
        categoryMap.get(oldCause.category.toLowerCase()) ||
        categoryMap.get(generateSlug(oldCause.category)) ||
        null;
    }

    // Parse numeric values
    const goal =
      typeof oldCause.goal === 'string'
        ? parseFloat(oldCause.goal.replace(/[^0-9.]/g, '')) || 0
        : oldCause.goal || 0;

    const raised =
      typeof oldCause.raised === 'string'
    ...
    try {
      await dataSource.query(
        ...
        [
          ...
          oldCause.image || null,
          oldCause.images
            ? JSON.stringify(oldCause.images)
            : oldCause.gallery
              ? JSON.stringify(oldCause.gallery.map((g: any) => g.url))
              : null,
          ...
          oldCause.contentBlocks
            ? JSON.stringify(oldCause.contentBlocks)
            : null,
...
async function migrateDonations() {
  ...
  const existingCount = await dataSource.query(`SELECT COUNT(*) FROM donation`);
  ...
  for (const oldDonation of oldDonations) {
    ...
    try {
      ...
      console.log(
        `  ❌ Error migrating donation "${transactionId}": ${error.message}`,
      );
      skipped++;
    }
  }
