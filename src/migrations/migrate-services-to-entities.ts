...
async function createDefaultCategories() {
  ...
  const defaultCategories = [
    ...
    },
    {
      name: 'Emergency',
      slug: 'emergency',
      description: 'Emergency response and relief services',
      color: '#ef4444',
      order: 3,
    },
    {
  ...
  console.log(
    `  ✅ Created ${defaultCategories.length} default service categories.`,
  );
}
...
async function migrateServices() {
  ...
  const existingCount = await dataSource.query(`SELECT COUNT(*) FROM service`);
    ...
    try {
      await dataSource.query(
        ...
        [
          ...
          oldService.contentBlocks
            ? JSON.stringify(oldService.contentBlocks)
            : null,
      ...
      console.log(
        `  ❌ Error migrating "${oldService.title}": ${error.message}`,
      );
      skipped++;
    }
  }
