...
export class PagesController {
    ...
    async getPageStatsLegacy(@Request() req: any) {
        if (!ADMIN_ROLES.includes(req.user?.role)) {
            return { data: { total: 0, published: 0, draft: 0, static: 0, dynamic: 0 } };
        }
        const stats = await this.pagesService.getPageStats();
        return { data: stats };
    }
    ...
    async createPageLegacy(@Body() dto: CreatePageDto, @Request() req: any) {
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins and editors can create pages' };
        }
        const page = await this.pagesService.createPage(dto);
        return { success: true, data: page };
    }
    ...
    async bulkUpdatePageStatusLegacy(@Body() dto: BulkPageStatusDto, @Request() req: any) {
        if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
    ...
    async getPageSections(@Param('id') id: string) {
        const sections = await this.pagesService.findSectionsByPageId(parseInt(id, 10));
    ...
    async createSection(
        ...
        const section = await this.pagesService.createSection(parseInt(id, 10), dto);
    ...
    async createSectionLegacy(
        ...
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Only admins and editors can create sections' };
        }
        const section = await this.pagesService.createSection(parseInt(id, 10), dto);
        return { success: true, data: section };
    }
    ...
    async updateSection(
        ...
        const section = await this.pagesService.updateSection(parseInt(id, 10), dto);
        return { success: true, data: section };
    }
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post('admin/sections/reorder')
    async reorderSectionsLegacy(@Body() dto: ReorderSectionsDto, @Request() req: any) {
        if (!EDITOR_ROLES.includes(req.user?.role)) {
            return { success: false, message: 'Access denied' };
        }
        await this.pagesService.reorderSections(dto);
