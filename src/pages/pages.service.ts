...
export class PagesService {
    ...
    async bulkUpdatePageStatus(dto: BulkPageStatusDto): Promise<{ success: boolean; count: number }> {
        const result = await this.pageRepository.update(
            { id: In(dto.ids) },
            { status: dto.status }
        );

        this.logger.log(`Bulk status update: ${result.affected} pages set to ${dto.status}`);
    ...
    async getPageStats(): Promise<{ total: number; published: number; draft: number; static: number; dynamic: number }> {
        const [total, published, draft, staticPages, dynamicPages] = await Promise.all([
        ...
        return { total, published, draft, static: staticPages, dynamic: dynamicPages };
    }
    ...
    async createSection(pageId: number, dto: CreateSectionDto): Promise<Section> {
        ...
        this.logger.log(`Section created: ${saved.name} for page ${page.title} (id: ${saved.id})`);
    ...
    async bulkUpdateSectionStatus(ids: number[], status: string): Promise<{ success: boolean; count: number }> {
    ...
    async reorderSections(dto: ReorderSectionsDto): Promise<{ success: boolean }> {
        for (let i = 0; i < dto.ids.length; i++) {
