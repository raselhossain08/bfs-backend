...
export class SuccessStoriesService {
    ...
    async create(dto: CreateSuccessStoryDto): Promise<SuccessStory> {
        ...
        const existing = await this.successStoryRepository.findOne({ where: { slug } });
        if (existing) {
    ...
    async findAll(query: SuccessStoryQueryDto) {
        const { page = 1, limit = 10, status, search, category, sortBy = 'order', sortOrder = 'ASC' } = query;
    ...
    async findBySlug(slug: string): Promise<SuccessStory> {
        const story = await this.successStoryRepository.findOne({ where: { slug } });
        if (!story) {
            throw new NotFoundException(`Success story with slug '${slug}' not found`);
        }
        return story;
    }
    ...
    async update(id: number, dto: UpdateSuccessStoryDto): Promise<SuccessStory> {
        ...
        if (dto.slug && dto.slug !== story.slug) {
            ...
            if (existing && existing.id !== id) {
                throw new BadRequestException(`Story with slug '${dto.slug}' already exists`);
            }
        }
