...
export class CausesController {
  ...
  async getAdminCategoriesLegacy(
    @Query() query: CauseCategoryQueryDto,
    @Request() req: any,
  ) {
  ...
  async createCategoryLegacy(
    @Body() dto: CreateCauseCategoryDto,
    @Request() req: any,
  ) {
  ...
  async updateCategory(
    ...
    const category = await this.causesService.updateCategory(
      parseInt(id, 10),
      dto,
    );
    return { success: true, data: category };
  ...
  async reorderCategoriesLegacy(
    @Body() dto: ReorderCauseCategoriesDto,
    @Request() req: any,
  ) {
    if (!EDITOR_ROLES.includes(req.user?.role)) {
      return { success: false, message: 'Access denied' };
  ...
  async createCauseLegacy(@Body() dto: CreateCauseDto, @Request() req: any) {
    if (!EDITOR_ROLES.includes(req.user?.role)) {
      return {
        success: false,
        message: 'Only admins and editors can create causes',
      };
    }
    const cause = await this.causesService.createCause({
      ...dto,
  ...
  @Roles(...EDITOR_ROLES)
  @Patch('admin/:id')
  async updateCause(@Param('id') id: string, @Body() dto: UpdateCauseDto) {
  ...
  async reorderCausesLegacy(
    @Body() dto: ReorderCausesDto,
    @Request() req: any,
  ) {
    if (!EDITOR_ROLES.includes(req.user?.role)) {
      return { success: false, message: 'Access denied' };
  ...
  async updateDonation(
    ...
    const donation = await this.causesService.updateDonation(
      parseInt(id, 10),
      dto,
    );
  ...
  @UseGuards(AuthGuard('jwt'))
  @Post('admin/donations/bulk-status')
  async bulkUpdateDonationStatusLegacy(
    @Body() dto: BulkDonationStatusDto,
    @Request() req: any,
  ) {
    if (!FULL_ADMIN_ROLES.includes(req.user?.role)) {
      return {
  ...
  async getDonationReceipt(
    ...
    const receipt = await this.causesService.generateReceipt(
      parseInt(id, 10),
      userId,
    );
  ...
  async emailDonationReceipt(@Param('id') id: string, @Request() req: any) {
    const userId = req.user?.userId || req.user?.sub || req.user?.id;
    if (!userId) {
  ...
  async generateAnnualSummary(
    ...
    const summary = await this.causesService.generateAnnualSummary(
      parseInt(year, 10),
      userId,
    );
