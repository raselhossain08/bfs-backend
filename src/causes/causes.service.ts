...
export class CausesService {
  ...
  async findPublicCauses(query: {
    ...
    if (isFeatured) {
      queryBuilder.andWhere('cause.isFeatured = :isFeatured', {
        isFeatured: true,
      });
    }
  ...
  async updateCause(id: number, dto: UpdateCauseDto): Promise<Cause> {
    ...
    if (dto.slug && dto.slug !== cause.slug) {
      ...
      if (existing) {
        throw new BadRequestException(
          `Cause with slug '${dto.slug}' already exists`,
        );
      }
    }
  ...
  async bulkUpdateStatus(
    ...
    this.logger.log(
      `Bulk status update: ${result.affected} causes set to ${dto.status}`,
    );
    ...
    return await this.dataSource.transaction(
      async (transactionalEntityManager) => {
        ...
        if (dto.causeId) {
          ...
          if (!cause) {
            throw new NotFoundException(
              `Cause with id ${dto.causeId} not found`,
            );
          }
        ...
        this.logger.log(
          `Donation created: $${saved.amount} for cause ${dto.causeId || 'General'}`,
        );

        // Track referral if user was referred (outside transaction to avoid deadlock)
        if (dto.donorId && dto.status === 'completed') {
          this.referralService
            .recordDonation(dto.donorId, dto.amount)
            .catch((error) => {
              this.logger.warn(
                `Failed to track referral for donation: ${error.message}`,
              );
            });
        }
  ...
  async bulkUpdateDonationStatus(
    ...
    const affectedCauseIds = [
      ...new Set(donations.filter((d) => d.causeId).map((d) => d.causeId)),
    ];
    ...
    for (const causeId of affectedCauseIds) {
      await this.updateCauseStats(causeId);
    }

    this.logger.log(
      `Bulk donation status update: ${result.affected} donations set to ${dto.status}`,
    );
  ...
  async getDonationStats(): Promise<{
    ...
    const [total, completed, pending, failed, totalSum, monthSum] =
      await Promise.all([
        this.donationRepository.count(),
  ...
  async getUserImpact(userId: number): Promise<{
    ...
    const previousMonthTotal = previousMonthDonations.reduce(
      (sum, d) => sum + parseFloat(d.amount.toString()),
      0,
    );
  ...
  async getUserDonations(
    ...
    if (query.causeId && query.causeId !== 'all') {
      queryBuilder.andWhere('donation.causeId = :causeId', {
        causeId: parseInt(query.causeId, 10),
      });
    }
...
<body>
    <div class="header">
        <div class="logo">Birdsfly Sangstha</div>
        <div class="org-info">
    ...
    <div class="footer">
        ...
        <p>Birdsfly Sangstha is a registered 501(c)(3) nonprofit organization.</p>
    </div>
</body>
</html>`;
