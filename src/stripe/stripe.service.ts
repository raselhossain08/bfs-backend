...
export class StripeService {
     ...
     */
    async handleCheckoutSessionCompleted(session: StripePaymentIntent['metadata'] & { sessionId: string; amount: number; currency: string }) {
        ...
        if (existingDonation) {
            this.logger.warn(`Donation already exists for session ${session.sessionId}, skipping duplicate`);
            return existingDonation;
        }

        let donorId: number | undefined = undefined;
        let autoRegisteredUser: { email: string; plainPassword: string; name: string } | null = null;
        ...
        if (session.donorId) {
            ...
            if (!isNaN(parsedId)) {
                ...
                if (user) {
                    donorId = parsedId;
                    isExistingUser = true;
                    this.logger.log(`Donation associated with existing user ID: ${donorId}`);
                    ...
                    if (!session.donorName || session.donorName === 'Anonymous') {
                        session.donorName = `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.email.split('@')[0];
                    }
                } else {
        ...
        if (!donorId && session.donorEmail) {
            const existingUser = await this.userRepository.findOne({ where: { email: session.donorEmail } });
            if (existingUser) {
                donorId = existingUser.id;
                isExistingUser = true;
                this.logger.log(`Found existing user by email: ${session.donorEmail} (ID: ${donorId})`);
            }
        }

        // Step 3: Auto-register if no user found and email is provided
        if (!donorId && session.donorEmail) {
            this.logger.log(`No account found for donor email: ${session.donorEmail}. Auto-registering...`);
            const result = await this.autoRegisterDonor(
                session.donorEmail,
                session.donorName,
            );
            if (result) {
                ...
                this.logger.log(`Auto-registered donor ${session.donorEmail} as user ID: ${donorId}`);
            } else {
        ...
        const donation = this.donationRepository.create({
            ...
            causeId: session.campaignId ? parseInt(session.campaignId, 10) : undefined,
        ...
        this.logger.log(`Donation created: ID=${savedDonation.id}, Amount=${savedDonation.amount} ${savedDonation.currency}, Donor=${savedDonation.donorId ? `User ${savedDonation.donorId}` : 'Anonymous'}`);
        ...
        if (autoRegisteredUser) {
            ...
            this.sendCredentialsEmail(
                ...
                this.logger.error(`Failed to send credentials email to ${autoRegisteredUser.email}: ${err.message}`, err.stack);
            });
            ...
            this.sendThankYouEmail(
                ...
                this.logger.error(`Failed to send thank you email to ${session.donorEmail}: ${err.message}`);
            });
        }
    ...
    private async autoRegisterDonor(
        ...
        try {
            ...
            if (existingUser) {
                this.logger.log(`User already exists with email ${email}, not auto-registering`);
            ...
            const firstName = nameParts[0] || email.split('@')[0].replace(/[^a-zA-Z]/g, '');
            ...
            this.logger.log(`Auto-registered donor ${email} as user ID: ${savedUser.id} with referral code: ${referralCode}`);

            return { userId: savedUser.id, plainPassword };
        } catch (error) {
            this.logger.error(`Auto-registration failed for ${email}: ${error.message}`, error.stack);
            return null;
        }
    }
    ...
    private generateThankYouHtml(name: string, donationAmount: string, causeName: string): string {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Thank You - BFS Foundation</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f4f5;">
     ...
     */
    async handlePaymentIntentFailed(session: StripePaymentIntent['metadata'] & { sessionId: string; amount: number }) {
