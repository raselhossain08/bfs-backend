...
export class AuthService {
    ...
    try {
      ...
      if (userData.referralCode) {
        try {
          ...
          this.logger.log(
            `Processed referral for new user ${user.id} with code ${userData.referralCode}`,
          );
        } catch (error) {
          // Log error but don't fail registration
          this.logger.warn(
            `Failed to process referral for user ${user.id}: ${error.message}`,
          );
        }
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to generate referral code for user ${user.id}: ${error.message}`,
        );
      }
  ...
  async updateProfile(userId: number, updateData: any) {
    ...
    const { password, resetToken, resetTokenExpiry, refreshToken, ...result } =
      updatedUser;
    return result;
  }
  ...
  async sendOTPLogin(email: string) {
    ...
    if (user.otpLoginExpiry && user.otpLoginCode) {
      const timeSinceLastOtp =
        Date.now() - (user.otpLoginExpiry.getTime() - 10 * 60 * 1000);
  ...
  async verifyOTPLogin(email: string, otp: string) {
    if (!otp || !/^\d{6}$/.test(otp)) {
      throw new BadRequestException(
        'Invalid OTP format. Please enter the 6-digit code.',
      );
    }
    ...
    if (
      ...
      Date.now() - user.otpAttemptsResetAt.getTime() <
        AuthService.OTP_ATTEMPTS_WINDOW_MS
    ) {
      throw new BadRequestException(
