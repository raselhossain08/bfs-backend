import { Controller, Get, Post, Patch, Delete, Body, Param, UseGuards, Request, Query, Ip, Headers } from '@nestjs/common';
...
export class SupportController {
    ...
    async getAnonymousHistory(@Param('sessionId') sessionId: string) {
        ...
        const messages = await this.supportService.getAnonymousChatHistory(sessionId);
    ...
    async sendMessage(@Request() req, @Body('message') message: string) {
        ...
        // Get bot response
        const botResponse = await this.supportService.getBotResponse(message, req.user.userId);
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post('tickets')
    async createTicket(@Request() req, @Body() ticketData: Partial<SupportTicket>) {
        const ticket = await this.supportService.createTicket(req.user.userId, ticketData);
        return { data: ticket };
    }
    ...
    async getTicketById(@Request() req, @Param('id') id: string) {
        const ticket = await this.supportService.getTicketById(parseInt(id, 10), req.user.userId);
        if (!ticket) {
    ...
    async getUserTicketReplies(@Request() req, @Param('id') id: string) {
        ...
        const replies = await this.supportService.getTicketReplies(parseInt(id, 10));
        return { data: replies };
    }
    ...
    async getMessagesByUserId(@Param('userId') userId: string) {
        const messages = await this.supportService.getMessagesByUserId(parseInt(userId, 10));
        return { data: messages };
    }
    ...
    async updateTicketStatus(
        ...
        const ticket = await this.supportService.updateTicketStatus(parseInt(id, 10), status);
        return { data: ticket };
    }
    ...
    async assignTicket(
        ...
        const ticket = await this.supportService.assignTicket(parseInt(id, 10), adminId);
        return { data: ticket };
    }
    ...
    async addTicketNote(
        ...
        const ticket = await this.supportService.addTicketNote(parseInt(id, 10), notes);
        return { data: ticket };
    }
    ...
    async adminReplyAnonymous(@Body() body: { sessionId: string; message: string }) {
