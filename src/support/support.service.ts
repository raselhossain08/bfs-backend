...
export class SupportService {
    ...
    await this.chatRepository.update(
      {
        userId,
        isRead: false,
        sender: In([MessageSender.SUPPORT, MessageSender.BOT]),
        source: MessageSource.AUTHENTICATED,
      },
      { isRead: true },
    );
  ...
  async getBotResponse(
    ...
    // Donation related
    if (
      /\b(donation|donate|give|contribute|receipt|tax)\b/.test(lowerMessage)
    ) {
    ...
    // Account related
    if (
      /\b(account|profile|password|login|email|settings)\b/.test(lowerMessage)
    ) {
    ...
    // Recurring donations
    if (
      /\b(recurring|monthly|weekly|subscription|automatic)\b/.test(lowerMessage)
    ) {
    ...
    // Impact related
    if (
      /\b(impact|where.*money|how.*help|difference|effect)\b/.test(lowerMessage)
    ) {
    ...
    // Technical issues
    if (
      /\b(error|bug|issue|problem|not working|broken|fail)\b/.test(lowerMessage)
    ) {
    ...
    // Escalation requests
    if (
      /\b(human|agent|person|support|help|talk|speak|representative)\b/.test(
        lowerMessage,
      )
    ) {
  ...
  async addTicketReply(
    ...
    const senderName =
      senderType === 'admin'
        ? savedReply.admin
          ? `${savedReply.admin.firstName || ''} ${savedReply.admin.lastName || ''}`.trim() ||
            'Support Team'
          : 'Support Team'
        : savedReply.user
          ? `${savedReply.user.firstName || ''} ${savedReply.user.lastName || ''}`.trim() ||
            'You'
          : 'You';
